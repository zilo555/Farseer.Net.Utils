...
namespace FS.Utils.Component.Open
{
    ...
    public class QQ : IOpenAuth
    {
        ...
        public QQ(string appID, string appKey, string authorizationCode, string redirectUri)
        {
            ...
            // 通过授权码、请求获取令牌 
            var url = $"https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&client_id={AppID}&client_secret={AppKey}&code={AuthorizationCode}&redirect_uri={redirectUri}";
        ...
        public string GetOpenID()
        {
            ...
            var json = Net.Get(url);
            return OpenID = Regex.Match(json, @"(?<=""openid"":"")[^""]*(?="")").Value;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public string GetUserInfo() => API("get_user_info", "");
        ...
        {
            var url = $"https://graph.qq.com/user/{apiName}";
        ...
        /// </summary>
        public static string GetAuthorizeUrl(string appID, string callBackUrl, string state) => $"https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id={appID}&redirect_uri={callBackUrl}&state={state}";
    }
}
#endif