    ...
    public class Sina
    {
        ...
        public Sina(string appID, string appKey, string authorizationCode, string redirectUri)
        {
            try
            {
                ...
                var param = string.Format("?grant_type=authorization_code&client_id={0}&client_secret={1}&code={2}&redirect_uri={3}", AppID, AppKey, AuthorizationCode, redirectUri);
        ...
        {
            var url = string.Format("https://api.weibo.com/2/users/{0}", apiName);
            var param = string.Format("?access_token={0}&uid={1}", AccessToken, Uid);
        ...
        {
            return string.Format("https://api.weibo.com/oauth2/authorize?response_type=code&client_id={0}&redirect_uri={1}&state={2}", appID, callBackUrl, state);
        }
    }
}
#endif