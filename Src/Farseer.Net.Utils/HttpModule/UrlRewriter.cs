...
namespace FS.Utils.HttpModule
{
    ...
    public class UrlRewriter : System.Web.IHttpModule, IRequiresSessionState
    {
        ...
        protected void UrlRewriter_AuthorizeRequest(object sender, EventArgs e)
        {
            ...
            // 循环所有重写规则
            foreach (var rule in WebUrlRewriterConfigs.ConfigEntity.RewriterRules)
            {
                // 取得规则地址
                var lookFor = "^" + ResolveUrl(appPath, rule.Url) + "$";
                ...
                // 取得重写规则地址
                var sendToUrl = ResolveUrl(appPath, re.Replace(url, rule.MapPath));
                // 写入追踪日志消息
